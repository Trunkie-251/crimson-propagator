...
/datum/controller/subsystem/communications/proc/make_announcement(mob/living/user, is_silicon, input, syndicate, list/players)
	...
	if(is_silicon)
		...
		priority_announce(html_decode(message_data["message"]), null, 'sound/misc/videoplayback.ogg', "[syndicate? "Syndicate " : ""]Captain", has_important_message = TRUE, players = players)
